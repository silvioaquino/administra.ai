    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="sticky top-0 z-10 bg-surface border-b border-border px-6 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-white">Correção de Produtos</h1>
      ...
      <div className="container mx-auto p-6 max-w-5xl">
        ...
        >
          ...
          <Button type="submit" className="bg-primary hover:bg-primary/90 text-white rounded-lg">
            Buscar
          </Button>
        </form>
        ...
        {loading ? (
          ...
          <div className="bg-surface rounded-2xl shadow-sm">
            <div className="flex h-80 flex-col items-center justify-center text-center p-6">
              <div className="w-16 h-16 bg-surface-2 rounded-full flex items-center justify-center mb-4">
                <PackageSearch className="h-8 w-8 text-muted-foreground/70" />
              </div>
              <p className="text-muted-foreground mb-2">Nenhum produto pendente de revisão</p>
          ...
          <div className="space-y-4">
            {produtos.map((item) => {
              ...
              return (
                <div key={item.id} className="bg-surface rounded-2xl shadow-sm p-5">
                  ...
                  <div className="mb-3">
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/70">Original</p>
                    <p className="text-sm text-white">{item.descricao}</p>
                  </div>

                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="sm:col-span-2">
                      <label className="text-xs font-medium text-muted-foreground">Nome normalizado</label>
                      <Input
                        value={nome}
                        onChange={(e) => aoMudar(item, "nomeNormalizado", e.target.value)}
                        className="mt-1 rounded-lg border-border focus:ring-primary focus:border-primary"
                      />
                    </div>
                    <div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <Button
                      onClick={() => salvar(item)}
                      disabled={!podeSalvar}
                      className="bg-primary hover:bg-primary/90 text-white rounded-lg disabled:opacity-50"
                    >
                      {savingId === item.id ? (
                    ...
                    <Button
                      ...
                      className="rounded-lg border-border hover:bg-surface-2 disabled:opacity-50"
