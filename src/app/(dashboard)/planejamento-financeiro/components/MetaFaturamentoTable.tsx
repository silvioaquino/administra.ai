...
interface MetaFaturamentoTableProps {
  dados: MetaFaturamentoRow[]
  anoAtual: number
...
export function MetaFaturamentoTable({ dados, anoAtual, onSalvar }: MetaFaturamentoTableProps) {
  ...
  useEffect(() => {
    if (dados && dados.length > 0) {
      const linhasAtualizadas = meses.map(m => {
        const encontrado = dados.find(d => d.mes === m.value)
        if (encontrado) {
      ...
      const primeiroComDados = dados.find(d =>
        (d.periodos.cafe ?? 0) > 0 ||
